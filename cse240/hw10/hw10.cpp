...
void changeNoOfCopies(Book* book,int copies_input)
{
	book->copies = copies_input;
}
...
{
	Container* tempList = list;					// work on a copy of 'list'
	Container* newContainer = new Container();  //creating a new container
	Book* newbook;								// creating a newbook
	// chose which lbbary the book is going to
	if (type == 0) // going to hayden
	{
		newbook = new Hayden(name_input, copies_input, type);
	}else{
		// going to noble 
		newbook  = new Noble(name_input, copies_input, type);
	}
	//put the book into the newcontainer
	newContainer->book = newbook;
	//set the bewcontainer next to null
	newContainer->next = NULL;
	// check if the contianer list is null
	if (list == NULL) // if the list is null set the list = new container
		list = newContainer;
	else{//if the list is not null add the newcotainer into the tail
...
void displayList()
{
	...
	//check if this list is null
	if (list == NULL)
		cout << "list is empty" << endl;
	//if the list is not null diaplay the list
	while (tempList != NULL)
	{
...
void save(string fileName)
{
	...
	ofstream myFileVar;
	//count how many book we gonna save
	while (templist != NULL)
	{
		templist = templist->next;
		count++;
	...
	if(myFileVar.is_open())
	{
		...
		while (templist != NULL)
		{
			myFileVar << templist->book->getName() << endl; //save the name
			myFileVar << templist->book->getCopies() << endl; // save the copy
			int typeint = int(templist->book->getLibraryType()); // cover libtype into a typeint
			myFileVar << typeint << endl;//save the typeint
			templist = templist->next;
		}
		myFileVar.close();
	}
...
void load(string fileName)
{
	...
	myFileVar.open(fileName);	
	if (myFileVar.is_open())
	{	// load the #number of book we gonna load
		myFileVar >> count;
		for (int i = 0; i < count; i++)
		{	// initialize 3 parameter
			string name; int copies; int libtype;
			myFileVar >> name; // load book name into name
			myFileVar >> copies; // load # of opies into copies
			myFileVar >> libtype; // load typeint into libtype
			libraryType type = libraryType(libtype);//cover int back to type
			addBook(name, copies, type);// use add fuction the add the new book
		}
		myFileVar.close();
	}
}
